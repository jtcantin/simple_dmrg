...
def build_random_mps(
    ...
    for i in range(num_sites):
        ...
        if i == 0:
            mps.append(rng.random((physical_dimension, bond_dimension)))
            mps[i] = np.expand_dims(mps[i], axis=0)
        elif i == num_sites - 1:
            mps.append(rng.random((bond_dimension, physical_dimension)))
            mps[i] = np.expand_dims(mps[i], axis=-1)
        else:
            mps.append(rng.random((bond_dimension, physical_dimension, bond_dimension)))
...
def mps_inner_product(
    ...
    for iiter in range(len(mps_ket)):
        ket_tensor = mps_ket[iiter]  # .copy()
        bra_tensor = mps_bra[iiter]  # .copy()
